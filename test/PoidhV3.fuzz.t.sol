...
contract PoidhV3FuzzTest is Test {
    ...
    function testFuzz_open_vote_outcome_matches_majority(uint96 issuerAmountRaw, uint96 contribAmountRaw, bool voteYes)
        public
    {
        uint256 min = poidh.MIN_BOUNTY_AMOUNT();
        uint256 issuerAmount = bound(uint256(issuerAmountRaw), min, 25 ether);
        uint256 contribAmount = bound(uint256(contribAmountRaw), 1 wei, 25 ether);
