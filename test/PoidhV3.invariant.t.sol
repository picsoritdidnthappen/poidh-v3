...
contract PoidhV3Handler is Test {
    ...
    constructor(PoidhV3 poidh_, PoidhClaimNFT nft_, address treasury_, address issuer_, address[] memory actors_) {
        poidh = poidh_;
        nft = nft_;
        treasury = treasury_;
        issuer = issuer_;
    ...
    function joinOpen(uint256 bountyIdRaw, uint96 amountRaw, uint256 actorSeed) external {
        ...
        uint256 amount = bound(uint256(amountRaw), 1 wei, 1 ether);
...
contract PoidhV3InvariantTest is StdInvariant, Test {
    ...
    function invariant_claimedBountiesFinalized_and_nftDelivered() public view {
        ...
        for (uint256 i = 0; i < bountyCount; i++) {
            (, address bountyIssuer,,, uint256 amount, address claimer,, uint256 claimId) = poidh.bounties(i);
    ...
    function invariant_openBountyAmountMatchesParticipantSums_unless_claimed() public view {
        ...
        for (uint256 i = 0; i < bountyCount; i++) {
            ...
            if (claimer != address(0) && claimer != bountyIssuer) continue; // claimed: participantAmounts are historical
